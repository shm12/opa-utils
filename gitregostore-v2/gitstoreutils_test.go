...
func BenchmarkXxx(b *testing.B) {
	for i := 0; i < b.N; i++ {
		s := newGitRegoStore("https://api.github.com/repos", "kubescape", "regolibrary", "releases", "latest/download", "dev", -1)
		err := s.setObjectsFromRepoOnce()
		if err != nil {
			b.Error(err)
		}
	}
...
func BenchmarkYyy(b *testing.B) {
	for i := 0; i < b.N; i++ {
		s := gitregostore.NewGitRegoStore("https://api.github.com/repos", "kubescape", "regolibrary", "git/trees", "", "dev", -1)
		err := s.SetObjectsFromRepoOnce()
		if err != nil {
			b.Error(err)
		}
